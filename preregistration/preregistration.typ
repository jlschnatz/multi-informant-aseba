...
To clarify the study design, particularly regarding the assessment waves in which the full item set of the Child Behavior Checklist and Youth Self Report @achenbach2001a were administered, the study manuals and data were accessed prior to preregistration without any data analysis. 
...
However, extensive meta-analytical empirical evidence has shown that discrepancies in reports from different informants (_informant discrepancies_) are common in these measures @delosreyes2015 @achenbach1987 @jungersen2023, leading to challenges in understanding and interpreting the results of the assessment procedures. 

Historically, informant discrepancies have been viewed as measurement error, treating them as interchangeable and informant-specific variance as nuisance @delosreyes2023a. This line of interpretation stems from the seminal work of #cite(<campbell1959a>, form: "prose") on multi-trait multi-method analysis, which has shaped both theoretical and empirical efforts to emphasize the common variance across informants @delosreyes2023. 
...
In addition to the need for instruments that capture both shared and informant-specific perspectives, there is also a growing demand for shortened tests that are feasible to implement in time-limited clinical settings @goetz2013. For example, within the widely used ASEBA system introduced above, the Brief Problem Monitor (BPM) has been developed as a shortened form of the ASEBA’s parent-, teacher-, and self-report forms to efficiently assess emotional and behavioral problems @achenbach2011. However, the BPM uses the same items across all informants, which may not be optimal for capturing meaningful informant-specific perspectives.This highlights the need for instruments that can effectively balance shared and informant-specific perspectives while also being practical for use in clinical settings.

== Objectives and Research Questions

The primary objective of this study is to develop a multi-informant subtest of the Achenbach System of Empirically Based Assessment (ASEBA) that balances shared and informant-specific perspectives, while also retaining a manageable number of items for economic administration in clinical settings. Specifically, we aim to identify a combination of items that function invariantly across informants (capturing common variance) and items that exhibit informant-specific functioning (capturing unique variance relevant to each informant). As part of our construction goal, we will attempt to ensure that all ASEBA clinical subscales are reflected in the final item set to additionally support the content validity of the subtest. To achieve this objective, we will leverage the combinatorial nature of item selection @schultze2017 using an automatic item selection approach to identify item sets that optimally satisfy a predefined set of construction criteria.

== Hypotheses

The following hypotheses will be tested in the current study:

1. $cal(H)_1$: The model fit of the developed subtest will meet a predefined set of Minimum Viable Criteria (MVC) (see section X for details relating to the definition of the MVC).

2. $cal(H)_2$: The subset of items intended to reflect the shared cross-informant perspective will demonstrate measurement invariance across different informants (child and parent).

3. $cal(H)_3$: The subset of items intended to reflect the informant-specific perspectives will demonstrate informant-specific functioning, indicating that these items capture unique variance relevant to each informant's perspective.
...
The dataset can be accessed after registration at the ICPSR website using a academic email adress. The data is available for scientific usage only and can be downloaded in various formats (SAS, SPSS, Stata, R, ASCII, Delimited) via this link https://doi.org/10.3886/ICPSR34368.v1. 
...
The dataset downloaded and first accessed on the #access_date.display("[month repr:long] [day], [year]").
...
The structure and contents (i.e., variables) of the dataset are known from the accompanying documentation, including variable lists, codebooks and study descriptions provided by the ICPSR. Relevant metadata on sampling, measurement time points, and instruments used are accessible, and the available variables have been reviewed in preparation for the planned analyses. No prior knowledge of statistical results or patterns related to the current hypotheses is available. 
...
Given that the items are rated on a three-point Likert-scale and measure clinical constructs that typically follow right-skewed distributions, it is highly likely that the assumption of multivariate normality will be violated. Therefore, we will use the robust maximum likelihood estimator (MLR) to account for non-normality.

== Handling of missing data

To address missing data in the dataset, we will use full information maximum likelihood (FIML) estimation @enders2001. This approach utilizes all available data, allowing for the inclusion of participants with incomplete data, and thereby avoiding the drawbacks of listwise or pairwise deletion.

== Measurement- and Structural Model

To construct a subtest of the ASEBA that integrates both shared and informant-specific perspectives, we will use a structural equation modeling (SEM) approach. Here, each manifest variable $X_(i j k l )$ refers to the $i$-th item ($i = 1, ..., i = I_j$) within the $j$-th clinical subscale of the ASEBA ($j = 1, ..., j = J$) for the $k$-th informant ($k in \{1, 2\}$) loading on the $l$ latent factor ($l in  \{ "CI-C", "CI-P", "IS-C", "IS-P"\}$). For simplicity, we will drop the subscript $l$ in the following description of the model and only refer to it when necessary.
...
The cross-informant perspective is modeled by latent factors for both child (CI-C) with manifest items of the YSR and parent (CI-P) with manifest items of the CBCL. As illustrated in Figure 1, items assigned to these latent factors must adhere to the constraint of configural invariance, meaning that same items must be selected for both informants. In constrast the informant-specific perspective is modeled by separate latent factors for child (IS-C) and parent (IS-P) with manifest items of the YSR and CBCL, respectively. Items for these latent factors can be freely selected, allowing for flexibility in capturing the unique informant-specific variance. Factor loadings the first item of each latent factor will be fixed to 1 to set the scale of the latent factors. The CI-P factor will be regressed on the CI-C factor, which is in line with the _CTC(M-1)_ model definition provided by #cite(<eid2000>, form: "prose"). All latent variances, covariances and regression paths will be freely estimated.

Regarding the number of manifest variables $X$ included in the model, we decided on $k = 2$ item per latent factor (as indicated in Figure 1) resulting in four items for each informant for each clinical subscale. In total, this equates to $8 dot 4 = 32$ items for each constructed subtest (child and parent informant). This corresponds to a significant reduction of the response burden of the original scale. Because a factor model including all eight clinical subscales at the same time would lead to a very complex model with a large number of parameters, we sequentially estimate the models for each subscale separately.
...
To select a set of items that meet our construction goal, we define an objective function composed of several criteria. The criteria are as follows:

1. _Model fit_

We will include model fit indices as criteria for the objective function. More specifically, we will use the following two indices to assess overall model fit: The sample-corrected robust Root Mean Square Error of Approximation @li2006.

$
R M S E A = sqrt(max(0, hat(F)_("ML")/(d f) - (hat(c))/(n - 1)))
$ <eq1>

where $hat(c)$ is a correction constant, $hat(F)_("ML")$ is the value of the minimized fit function, $d f$ are the degress of freedom and $n$ is the sample size @brosseau-liard2012. This corresponds to the "rmsea.robust" criterion in the _lavaan_ package @rosseel2012.  

Additionally, we will use the Standardized Root Mean Square Residual @bentler1995

$
S R M R = sqrt(2/(p (p + 1)) sum_(u = 1)^p sum_(v=1)^p ((s_(u v) - hat(sigma)_(u v)) / (s_(u u) s_(v v)))^2)
$ <eq2>

where $p$ is the number of manifest variables in the model, $s_(u v)$ is the sample covariance between manifest variables $u$ and $v$, and $hat(sigma)_(u v)$ is the estimated covariance between manifest variables $u$ and $v$. This corresponds to the "srmr" criterion in the _lavaan_ package @rosseel2012.

2. _Reliability of each subscale $overline(omega)$_

As a criterion for the average reliability across the four latent factors, we will calculate the average composite reliability $overline(omega)$. This is given by the following formula:
...
Highlighted in orange in the model in @fig2, the regression coefficient $gamma$ from the cross-informant parent factor (CI-P) to the cross-informant child factor (CI-C) will be included as a criterion in the objective function. 

4. _Latent correlation_ $phi$

Finally, the latent correlation $phi$ highlighted in blue in @fig2
between the informant-specific factors (IS-C and IS-P) will be included as a criterion in the objective function. 
...
$k = 5000$ random solutions sampled from the search space of all possible item combinations @schultze2021 @schultze2022. The mean and standard deviation of the top 10% of valid solutions will be used for the transformation.

The combined empirical objective function $f$ is then a weighted sum of the set of transformed criteria $cal(X): \{ "RMSEA", "SRMR", omega, phi, gamma\}$:

$
f = sum_(q = 1)^r w_q dot Phi(x_q\; hat(mu)_(x_q), hat(sigma)_(x_q))
$ <eq4>

with weights $cal(W)$ set as $w_("RMSEA") = w_("SRMR") = w_phi = w_gamma = 1/6$ and  $w_(omega) = 1/3$, reflecting their relative importance.


== Model Estimation

Because the search space of all possible item combinations is feasable for a brute-force search, we will use an exhaustive search to identify the item combination that optimally satisfies the criteria of the empirical objective function $f$. As a rationale for the brute-force search, we note that the number of combinations is feasible to compute exhaustively. For the clinical subscale with the most number of items, _Aggressive Behavior (AB)_ with $I_(A B) = 13$ items, we select $g_l = 2$ items per factor and have $L = 4$ latent factors in total (CI-C, CI-P, IS-C, IS-P). Because the items of the cross-informant factors are restricted to adhere to configural invariance (i.e., same items for both informants), the number of combinations is given by

$
product_(l = 1)^(L-1) vec(I_(A B), g) = 474552
$ <eq5>

which is a manageable number of combinations to compute exhaustively. For the remaining clinical subscales, the number of combinations is even smaller, because they have fewer items.

To estimate the structural equation models, we will use the robust maximum likelihood estimator (MLR), as the data will likely violate the assumption of multivariate normality. For this the `stuart` R-package @schultze2023 to perform the search and model estimation, which uses the `lavaan` package @rosseel2012 as a backend will be employed.
...
In order to assess the overall quality of the constructed subtest ($cal(H)_1$), we will define a set of Minimum Viable Criteria (MVC) based on the criteria included in the empirical objective function $f$. The MVC will serve as benchmarks that the constructed subtest _must_ meet to be considered acceptable. 
This will determined without interential testing, but rather by comparing the estimated values of the criteria to the determined cutoff values of the MVC. If a subscale does not meet one of the criteria, we will run through a diagnostic algorithm that is described in more detail in the next section. 

_Model fit Criteria_
...
_Latent regression weight_ $gamma$:
...
5. If the average reliability $overline(omega)$ is too low and does not meet the MVC, we will check, which of the four factors falls below the reliability threshold. If one of the cross-informant factors (CI-C, CI-P) falls below it, we will drop both factors. If one of the informant-specific factors (IS-C, IS-P) falls below it, we will drop only the respective factors. If at least one of both cross-informant and informant-specific factors falls below the threshold, we will drop the entire clinical subscale from the item selection procedure.

In cases where one or more factors are dropped from the model, we will rerun the item selection procedure with the reduced model specification. In potential cases that were not preregistered, we exploratory post-hoc analyses will be conducted and stated as such. Any findings from these exploratory post-hoc analyses needs to be verified by new data.
...
Hypothesis 2 will be tested by assessing the measurement invariance of the cross-informant factors across the child and parent informants. By specification of the measurement model, these factors are already constrained to configural invariance. We will sequentially test for more restrictive levels of measurement invariance (metric, scalar, strict). To test the levels of invariance, we will per default refer to information information criteria (AIC, BIC, aBIC; see @eq7 - @eq9) to assess whether the more restrictive models leads to a substantial decrease in model fit. If the information criteria do not agree uniamously, we will additionally use a $chi^2$ difference test to test whether the equivalence in model fit between the less and more restrictive model must be rejected.
...
To test the informant-specific functioning of items selected for the informant-specific factors, we will let items selected for the informant specific factors (IS-C, IS-P) load onto their respective cross-informant factor counterpart and remove the informant-specific factors from the model completely. We will then assess whether the model fit in terms of RMSEA and SRMR where cutoffs will again be determined using the _ezCutoffs_ approach described above. However, cutoffs determined in this way previously cannot be used and must be newly calculated, because the model structure has changed. If the model fit meets the cutoffs, we will reject the hypothesis of informant-specific functioning.
